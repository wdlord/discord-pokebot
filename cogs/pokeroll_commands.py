...
from pokeapi import get_pokemon, type_to_color, type_to_emoji
...
class PokemonSearchCard(discord.ui.View):
    ...
    def make_card(self) -> discord.Embed:
        ...
        desc = (
            f"{''.join(type_to_emoji[t['type']['name']] for t in self.pokemon['types'])}"
...
async def roll_pokemon(interaction: discord.Interaction, remaining_rolls: int):
    ...
    desc = (
        f"{''.join(type_to_emoji[t['type']['name']] for t in pokemon['types'])}"
    )
